...
public class SectionAdd extends Frame {
    ...
    private Consumer<Section> onSectionAdded;

    // Accepted a functional callback parameter to pass the constructed Section instance back
    public SectionAdd(String title, Dimension size, Palette palette, Label label, Consumer<Section> onSectionAdded) {
        ...
        Pane content = new Pane(getDimension(), getPalette(), getLabel()) {
            {
                ...
                btnAdd.addActionListener(e -> {
                    ...
                    String sectionCode = txtSectionCode.getText().trim();
                    
                    // Validation fallback to prevent adding blank structural rows
                    if (!sectionName.isEmpty()) {
                        if (onSectionAdded != null) {
                            // Instantiate Section with both Name and Code arguments 
                            // (Adjust constructor arguments to match your exact Section model attributes if needed)
                            Section newSection = new Section(1, sectionName, sectionCode, new ArrayList<>()); 
                            onSectionAdded.accept(newSection);
                        }
                    }
                    dispose(); // Close window cleanly
                });
